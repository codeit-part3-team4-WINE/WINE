...
export default function RecommendedWineItem({
  ...
  return (
    <Link
      className='inset-shadow-md flex h-[15rem] w-[22rem] flex-shrink-0 cursor-pointer items-center justify-between gap-4 overflow-y-hidden rounded-2xl bg-white px-10 md:h-[18rem] md:w-[26rem] md:px-5'
      href={`/wines/${id}`}
    >
      {imageSrc ? (
        <div className='flex h-[17rem] w-full max-w-[6rem] items-center justify-center pt-5 md:h-[19rem] md:max-w-[10rem] md:min-w-[8rem]'>
          <Image
            alt='wine'
